    ...
    it('verifies history after appointment', () => {
        cy.fixture('appointments').then((appointment) => {   
            ...
            cy.get('.panel-info').should('be.visible').then((historyPanel)=>{
                cy.get(historyPanel).find('.panel-heading').should('have.text',appointment[testCaseNumber].visitDate)
                cy.get(historyPanel).find('#facility').should('have.text',appointment[testCaseNumber].facility)
                cy.get(historyPanel).find('#hospital_readmission').should('have.text',appointment[testCaseNumber].hospitalReadmission)   
                cy.get(historyPanel).find('#program').should('have.text',appointment[testCaseNumber].healtcareProgram)  
                cy.get(historyPanel).find('#comment').should('have.text',appointment[testCaseNumber].comment)
